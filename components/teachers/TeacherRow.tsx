...
import { MoreHorizontalIcon, PencilIcon, ChalkboardTeacherIcon } from '../icons';
...
export const TeacherRow: React.FC<TeacherRowProps> = ({ teacher, onEdit, currentUser }) => {
    const statusColors = {
                    ...
                    {isMenuOpen && (
                        <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-10 border border-slate-200/80">
                            <div className="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                                {canEdit && (
                                    <button onClick={() => { onEdit(teacher); setIsMenuOpen(false); }} className="w-full text-left flex items-center px-4 py-2 text-sm text-slate-700 hover:bg-slate-100" role="menuitem">
